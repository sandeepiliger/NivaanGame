...
/**
 * One-shot diagnostic for the parent zone's "Test voice" button. Speaks a
 * fixed test line and reports exactly what happened — installed voice
 * count, which voice got picked, whether the utterance actually started
 * and ended, and any error — so a silent-voice report from a real device
...
export function diagnoseVoice() {
  return new Promise((resolve) => {
    ...
    if (!info.hasSpeechSynthesis) {
      resolve(info);
      return;
    }
    const available = speechSynthesis.getVoices();
    info.voiceCount = available.length;
    info.voices = available
      .slice(0, 6)
    ...
    try {
      speechSynthesis.cancel();
      const utter = new SpeechSynthesisUtterance('Testing. Can you hear my voice?');
      if (voice) {
        utter.voice = voice;
        utter.lang = voice.lang;
      }
    ...
    // In case neither onend nor onerror ever fires (a real possibility if
    // there's simply no TTS engine at all), don't hang forever.
